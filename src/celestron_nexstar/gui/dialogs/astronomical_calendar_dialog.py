    ...
    def _load_astropixels_events(self, start_date: datetime) -> None:
        ...
        try:
            ...
            async def _load() -> None:
                ...
                cached_events = await get_cached_astropixels_events(start_date, end_date, tz_offset)

                # Only fetch if we don't have enough cached events or if we're near year end
                # Check if we need to fetch for current year
                needs_fetch = len(cached_events) == 0
                if not needs_fetch:
                    # Check if we have events for the current year
                    year_events = [e for e in cached_events if e.date.year == year]
                    needs_fetch = len(year_events) == 0

                if needs_fetch:
                    logger.info(f"Fetching AstroPixels almanac for {year} ({tz_str})")
                    await cache_astropixels_events(year, tz_str, force_refresh=False)

                # Also cache next year if we're near the end of the year
                if start_date.month >= 11:
                    next_year_events = [e for e in cached_events if e.date.year == year + 1]
                    if len(next_year_events) == 0:
                        logger.info(f"Fetching AstroPixels almanac for {year + 1} ({tz_str})")
                        await cache_astropixels_events(year + 1, tz_str, force_refresh=False)

                # Get cached events with timezone offset (after potential fetch)
                cached_events = await get_cached_astropixels_events(start_date, end_date, tz_offset)
                ...
                # Update calendar formatting after loading all events (only once)
    ...
    def _load_eclipses(self, location: ObserverLocation | Any, start_date: datetime) -> None:
        ...
        try:
            ...
            async def _load() -> None:
                ...
                # Load lunar eclipses
                lunar_eclipses = get_next_lunar_eclipse(location, years_ahead=1)
                ...
                # Load solar eclipses
                solar_eclipses = get_next_solar_eclipse(location, years_ahead=1)
