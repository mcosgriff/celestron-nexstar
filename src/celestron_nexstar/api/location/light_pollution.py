...
from celestron_nexstar.api.database.duckdb_access import get_bortle_characteristics
...
def _get_bortle_characteristics(bortle_class: BortleClass) -> dict[str, Any]:
    """
    ...
    model = get_bortle_characteristics(int(bortle_class.value))
    if model is None:
        raise DatabaseError(
...
def _create_light_pollution_data(sqm: float, source: str | None = None, cached: bool = False) -> LightPollutionData:
    ...
    chars = _get_bortle_characteristics(bortle)
...
async def _fetch_sqm(lat: float, lon: float) -> float | None:
    ...
    try:
        ...
        sqm = get_sqm_from_database(lat, lon)
        if sqm is not None:
