    ...
    for url in urls:
        try:
            ...
            if response.status_code != 200:
                _debug_print(f"[constellation_images] Failed to download from {url}: HTTP {response.status_code}")
                logger.info(f"Failed to download from {url}: HTTP {response.status_code}")
                continue
            ...
            if "image/svg" not in content_type and "text/html" in content_type:
                ...
                if direct_url:
                    ...
                    # Try the direct URL
                    direct_response = requests.get(direct_url, headers=headers, timeout=10)
                    if direct_response.status_code == 200:
                        data = direct_response.content
                        _debug_print(
                        ...
                        _debug_print(
                            f"[constellation_images] Failed to download from direct URL: HTTP {direct_response.status_code}"
                        )
                        logger.info(f"Failed to download from direct URL: HTTP {direct_response.status_code}")
                        continue
                else:
                    _debug_print(
