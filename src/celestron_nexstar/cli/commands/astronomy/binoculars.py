...
from celestron_nexstar.api.astronomy.meteor_showers import get_active_showers, get_peak_showers, get_radiant_position
...
app = typer.Typer(help="Binocular viewing commands", cls=SortedCommandsGroup)
console = Console()
...
async def _show_tonight_content(binoculars: str, output_console: Console | FileConsole) -> None:
    ...
    try:
        ...
        if iss_passes:
            ...
            output_console.print(
                "[dim]Quality based on maximum altitude: Excellent >70°, Very Good >50°, Good >30°[/dim]"
            )
        else:
        ...
        with get_db_session() as db_session:
            visible_constellations = get_visible_constellations(db_session, lat, lon, midnight, min_altitude_deg=10.0)
        ...
        if fully_visible:
            ...
            for constellation, alt, az in fully_visible[:12]:  # Top 12
                ...
                if constellation.season != current_season:
                    season_display = (
                        f"{constellation.season} (best in {constellation.season.lower()}, visible out of season)"
                    )
        ...
        if partially_visible:
            ...
            table_partial.add_column("Season", style="dim")
        ...
        with get_db_session() as db_session:
            visible_asterisms = get_visible_asterisms(db_session, lat, lon, midnight, min_altitude_deg=20.0)
...
def show_checklist(
    ...
    """Generate pre-observation checklist for binocular viewing."""
    checklist = generate_observation_checklist(equipment_type="binoculars")

    if export:
        export_path_obj = (
            Path(export_path) if export_path else Path(f"binoculars_checklist_{datetime.now().strftime('%Y%m%d')}.txt")
        )
        file_console = create_file_console()
...
def _show_checklist_content(output_console: Console | FileConsole, checklist: list[str]) -> None:
    ...
    output_console.print("\n[bold cyan]Pre-Observation Checklist (Binoculars)[/bold cyan]\n")
...
def show_log_template(
    ...
    if export:
        export_path_obj = (
            Path(export_path) if export_path else Path(f"binoculars_log_{datetime.now().strftime('%Y%m%d')}.txt")
        )
        file_console = create_file_console()
